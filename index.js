...
function filldata(ctr){
    ...
    for(j=0;j<5;j++){
        ...
        document.getElementById(j).appendChild(ne)
        var ne=document.createElement("h5")
        ne.innerText +="email : "+ inidata[j].email
        document.getElementById(j).appendChild(ne)
    }

    var ge=document.createElement("h5")
    ge.innerText="Page Number "+(~~((ctr+6)/5))+"/"+(data.length/5)
    document.getElementById("k").appendChild(ge)
